...
while ! nc -z database_service 5432; do
  sleep 0.1
done
...
# this bash script checks port number 5432 of container named test-db
# to ascertain that it is open. this means that the postgres server is up and running
# test-db is what i named the postgres database container.
